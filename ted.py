...
class TED(nn.Module):
    ...
    def __init__(self, inputchannels=32, outputchannels=16):
        super(TED, self).__init__()
        self.block_1 = DoubleConvBlock(inputchannels, 16, 16, stride=2,)
        ...
        # for v1-v6
        # self.conv1x1_4 = nn.Conv2d(4, 16, kernel_size=1, stride=1, padding=0)
        # for v7
        self.conv1x1_4 = nn.Conv2d(4, outputchannels, kernel_size=1, stride=1, padding=0, bias= False)
    ...
    def forward(self, x, single_test=False):
        ...
        # batch = x.size(0)
        # original_m = x.size(3)
        # pad_size = (4 - original_m % 4) % 4  # 计算需要填充的大小，使 m 能够被 4 整除
        # x = F.pad(x, (0, pad_size))  # 在 m 维度上进行零填充
        
        B, _, freq, time = x.size()
        original_freq = freq
        original_time = time
        
        pad_value = 4
        # 计算需要填充的大小，使 freq 和 time 维度均能够被 pad_value 整除
        pad_size_time = (pad_value - time % pad_value) % pad_value
        pad_size_freq = (pad_value - freq % pad_value) % pad_value
        
        # 在 freq 和 time 维度上进行零填充
        x = F.pad(x, (0, pad_size_time, 0, pad_size_freq))
        ...
        #[batch, 512, m]
        results = self.conv1x1_4(results).squeeze(dim=2)

        # 恢复原来的大小
        results = results[:, :, :, :original_time]
        results = results[:, :, :original_freq, :]
