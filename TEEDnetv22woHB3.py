...
class Downsampling(nn.Module):
    def __init__(self, input_channels, emb_channels, output_channels, kernel_sizes_full, kernel_sizes_mid, kernel_sizes_tiny):
        ...
        self.hb_full = nn.Conv2d(emb_channels, emb_channels, 1)
        self.hb_mid = nn.Conv2d(emb_channels, emb_channels, 1)
        self.hb_tiny = nn.Conv2d(emb_channels, emb_channels, 1)
...
class GRUAlongFrequency(nn.Module):
    def __init__(self, Time, hidden_size, num_layers=1):
        ...
        self.gru = nn.GRU(input_size=Time, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
        self.proj = nn.Linear(hidden_size, Time)

    def forward(self, x):
        B, C, F, T = x.size()
        x = x.permute(0, 1, 2, 3).contiguous()  # (B, C, F, T)
        x = x.view(B * C, F, T)  # (B*C, F, T)
        output, _ = self.gru(x)  # (B*C, F, hidden_size)
        output = self.proj(output)  # (B*C, F, T)
        output = output.view(B, C, F, T)  # (B, C, F, T)
        return output
...
# 主网络架构
class HAUnetv2woHB3(nn.Module):
    def __init__(self, Time=128, Frequency=360):
        super(HAUnetv2woHB3, self).__init__()
